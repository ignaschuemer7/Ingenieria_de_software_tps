...
newT amountBays height r | amountBays >= 0 && height >= 0 = Tru (replicate amountBays (newS height)) r
                         | otherwise = error "The amount of bays and the height mustn't be negative"
...
loadT (Tru bays r) p | null availableBays = Tru bays r -- no hay ningun stack en el que se puede cargar un palet
                     | otherwise = Tru (updateBays bays (head availableBays) p) r
                       where 
...
updateBays :: [Stack] -> Int -> Palet -> [Stack]
